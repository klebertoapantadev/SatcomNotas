...
AUTOR: Antigravity (Optimización Final)
FECHA: 2026-05-04
PROPÓSITO: Monitoreo de consultas recurrentes cruzado con la última actividad del emisor.
=========================================================================================
*/
...
BEGIN
    ...
    ---------------------------------------------------------------------------
    -- 1. PRE-PROCESAMIENTO DE ACTIVIDAD DE EMISORES
    -- Obtenemos la última fecha de autorización global por emisor (evita duplicados por establecimiento)
    ---------------------------------------------------------------------------
    ;WITH CTE_Actividad AS (
        SELECT 
            ID_Emisor, 
            MAX(Ultima_Fecha_Autorizacion) AS Max_Ultima_Fecha_Autorizacion
        FROM sat_logging..log_actividad_emisor WITH(NOLOCK)
        GROUP BY ID_Emisor
    )
    ---------------------------------------------------------------------------
    -- 2. CONSULTA DE RESUMEN (RECURRENTES)
    ---------------------------------------------------------------------------
    SELECT 
        ...
        L.lc_nombre_sp,
        L.lc_emisor,
        -- Lógica de parámetros según rama original para mantener compatibilidad
        CASE 
            WHEN @Emisor IS NOT NULL AND @SP IS NULL THEN dbo.fn_get_scrip_sp(L.lc_parametros) 
            ELSE L.lc_parametros 
        END AS lc_parametros,
        L.lc_usuario,
        A.Max_Ultima_Fecha_Autorizacion AS Ultima_Fecha_Trx_Autorizada
    ...
    LEFT JOIN CTE_Actividad A ON A.ID_Emisor = L.lc_emisor
    ...
    GROUP BY 
        ...
        END,
        L.lc_usuario,
        A.Max_Ultima_Fecha_Autorizacion
    HAVING COUNT(1) > 3
    ORDER BY NumConsultas DESC;

    ---------------------------------------------------------------------------
    -- 3. CONSULTAS DE DETALLE (Mantiene lógica de ramas originales)
    ---------------------------------------------------------------------------
    ...
    END
    -- Rama: General (Sin parámetros específicos o solo Emisor)
    ELSE IF (@Emisor IS NULL OR (@Emisor IS NOT NULL AND @SP IS NULL))
    BEGIN
        SELECT 
